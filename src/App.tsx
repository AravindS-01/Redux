...
import { removeMeal } from './redux/reducer';
...
function App() {
  ...
  const removeItem = useCallback((dish:Dish)=>{
       const reduceItem = {
        ...dish,
        numberOfItem: Number(dish.numberOfItem) - 1
       };
  ...
  return (
    <>
      ...
      <div className='card'>
        ...
        {selector && !!selector.dishes.length && selector.dishes.map(item => {
          return (
            <>
              <li style={{ display: 'flex',justifyContent:'space-between' ,alignItems:'center'}}>
                <div className='dispaly'>
                  ...
                  <div style={{ display: 'flex' }}>
                    ...
                    <span style={{ fontWeight: "bold" }}>{item.numberOfItem}</span></div>
                </div>
                <button className='reduceButton' onClick={() =>{removeItem(item)}}>Reduce Item in Cart</button>
              </li>
              <hr />
            </>
          )
        ...
        <p style={{ fontSize: '24px', fontWeight: 700, display: 'flex', flexDirection: 'column' }}>
